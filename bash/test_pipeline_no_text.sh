...
# Usage: bash bash/test_pipeline_no_text.sh <path_to_pkl_file>
...
if [ -z "$PKL_PATH" ]; then
  ...
  if [ -z "$LATEST_PKL" ]; then
    ...
    echo "Usage: bash bash/test_pipeline_no_text.sh <path_to_pkl_file>"
    exit 1
  fi
  PKL_PATH=$LATEST_PKL
...
# Call the python script in tests/
